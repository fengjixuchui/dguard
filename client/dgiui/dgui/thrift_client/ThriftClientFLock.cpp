...
namespace thrift_client
{
    ...
    bool ClientFLock::setStateById(const std::string& _sid, const std::string& _flockId, const FLockState& _newState)
    {
        std::shared_ptr<TTransport> socket(new TSocket(m_host, m_port));
        std::shared_ptr<TTransport> transport(new TBufferedTransport(socket));
        std::shared_ptr<TProtocol> protocol(new TBinaryProtocol(transport));
        ...
        try
        {
            ...
            transport->open();
            client.setStateById(response, _sid, _flockId, newState);
            transport->close();
