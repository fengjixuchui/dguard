...
#pragma alloc_text(PAGE, SziFileEraseUnloadRoutine)
#pragma alloc_text(PAGE, SziFileEraseFltUnload)
#pragma alloc_text(PAGE, SziFileEraseInstanceQueryTeardown)
#pragma alloc_text(PAGE, SziFileEraseInstanceSetup)
#pragma alloc_text(PAGE, SziFileEraseInstanceTeardownStart)
#pragma alloc_text(PAGE, SziFileEraseInstanceTeardownComplete)
#pragma alloc_text(PAGE, SziFileEraseAllocateContext)
#pragma alloc_text(PAGE, SziFileErasePreSetInfoCallback)
#pragma alloc_text(PAGE, SziFileErasePreCleanupCallback)
#pragma alloc_text(PAGE, SziFileErasePostCleanupCallback)
//#pragma alloc_text(PAGE, SziFileErasePostSetInfoCallback)   //!!! NONPage
//#pragma alloc_text(PAGE, SziFileEraseDeviceControlRoutine)	//!!! NONPage
//#pragma alloc_text(PAGE, SziFileEraseSecureDeleteFile)		//!!! NONPage
//#pragma alloc_text(PAGE, SziFileEraseFltSecureDeleteFile)	//!!! NONPage
#endif
...
CONST FLT_REGISTRATION FilterRegistration = {
    ...
    NULL,/*SziFileEraseFltUnload*/              //  MiniFilterUnload

    SziFileEraseInstanceSetup,                    //  InstanceSetup
    SziFileEraseInstanceQueryTeardown,            //  InstanceQueryTeardown
    SziFileEraseInstanceTeardownStart,            //  InstanceTeardownStart
    SziFileEraseInstanceTeardownComplete,         //  InstanceTeardownComplete
...
NTSTATUS
SziFileEraseIrpSuccessHandler(
	PDEVICE_OBJECT Fdo, 
	PIRP Irp
	)
{
	UNREFERENCED_PARAMETER(Fdo);
	return SziFileEraseCompleteIrp(Irp, STATUS_SUCCESS, 0);
}
    ...
    )
{
	...
	DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = SziFileEraseDeviceControlRoutine;
	DriverObject->MajorFunction[IRP_MJ_CREATE] = SziFileEraseIrpSuccessHandler;
	DriverObject->MajorFunction[IRP_MJ_CLOSE] = SziFileEraseIrpSuccessHandler;
	DriverObject->MajorFunction[IRP_MJ_READ] = SziFileEraseIrpSuccessHandler;
	DriverObject->MajorFunction[IRP_MJ_WRITE] = SziFileEraseIrpSuccessHandler;

#ifdef _DEBUG
	gTraceFlags = PTDBG_TRACE_ROUTINES | PTDBG_TRACE_ERRORS | PTDBG_TRACE_OPERATION_STATUS | PTDBG_TRACE_VALUE | PTDBG_TRACE_FILE;
	//DriverObject->DriverUnload = SziFileEraseUnloadRoutine;
#endif // _DEBUG
	
	PT_DBG_PRINT(PTDBG_TRACE_VALUE, ("FileErase!gTraceFlags: 0x%X\n", gTraceFlags));

	status = SziCreateUnicodeStringFromUnicodeString(&gUserContext.RegistryPath, RegistryPath, DF_STRING_POOL_TAG);
...
NTSTATUS 
SziFileEraseDeviceControlRoutine(
	PDEVICE_OBJECT Fdo, 
	PIRP Irp
	)
{
			...
			if(bufferInSize == sizeof(USER_ERASEFILE_CONTEXT) && method == METHOD_BUFFERED) {
					...
					__try {
						...
						if(hEvent && fileName && length > 0 && length < MAX_PATH) {
							status = ObReferenceObjectByHandle(hEvent, EVENT_MODIFY_STATE, *ExEventObjectType, Irp->RequestorMode, (PVOID*)&contextQueue->Event, NULL);
							if(NT_SUCCESS(status)) {
					...
					if(NT_SUCCESS(status)) {
						...
						if(contextQueue->WorkItem) {
							IoQueueWorkItem(contextQueue->WorkItem, SziFileEraseDiskSecureWorkItem, DelayedWorkQueue, contextQueue);
...
_error:
	...
	return SziFileEraseCompleteIrp(Irp, status, bytesTxd); // Завершение IRP;
}

VOID 
SziFileEraseUnloadRoutine(
    ...
    )
{
	...
	if(FlagOn(Data->Iopb->Parameters.Create.Options, FILE_DELETE_ON_CLOSE)) {
		status = SziFileEraseAllocateContext(&streamContext);
    ...
    )
{
	...
	if(NT_SUCCESS(Data->IoStatus.Status) &&	(STATUS_REPARSE != Data->IoStatus.Status)) {
		...
		status = SziFileEraseGetOrSetContext(FltObjects, Data->Iopb->TargetFileObject, &streamContext);
		if (NT_SUCCESS(status))	{
    ...
    )
{
	...
	if (Data->Iopb->Parameters.SetFileInformation.FileInformationClass == FileDispositionInformation) {
		if(FltObjects->Transaction != NULL) {
			...
			status = SziFileEraseFltSecureDeleteFile(Data, FltObjects, FALSE);
			...
			if(!(InterlockedIncrement(&streamContext->NumOps) > 1)) {
				*CompletionContext = (PVOID)streamContext;
				...
				PT_DBG_FILE("%s: FltReleaseContext: NumOps %d Erase %d OnClose %d IsE %d", __FUNCTION__, streamContext->NumOps, streamContext->Erase, streamContext->DeleteOnClose, streamContext->IsErase);
    ...
    )
{
	...
	if (FltObjects->Transaction == NULL) {
		...
		if(streamContext->DeleteOnClose || streamContext->Erase == 1) {
			status = SziFileEraseFltSecureDeleteFile(Data, FltObjects, streamContext->DeleteOnClose);
    ...
    )
{
	...
	PT_DBG_FILE("%s: Context %p", __FUNCTION__, streamContext);
	PT_DBG_FILE("%s: ContextFile %wZ", __FUNCTION__, &Data->Iopb->TargetFileObject->FileName);
	//FltDeleteContext(CompletionContext);
	FltReleaseContext(CompletionContext);
    return FLT_POSTOP_FINISHED_PROCESSING;
}
