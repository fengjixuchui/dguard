...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, FEraseFltSecureOverwrite)
	...
	_In_ UCHAR Mask)
{
	...
	//status = FEraseFltSecureOverwrite(Context, offset, sizeof(UCHAR), Count, Mask);
	//if(!NT_SUCCESS(status)) {
	//	PT_DBG_FILE("Status 0x%X %s:%d", status, __FILE__, __LINE__);
	//	goto _error;
	//}
	...
	while(offset.QuadPart < Context->InfoFile.AllocationSize.QuadPart)
	{
		...
		status = FEraseFltSecureOverwrite(Context, offset, bytesToWrite, Count, Mask);
	...
	_In_ UCHAR Mask) 
{
	...
	status = FEraseFltGetFileClusters(Context, &vcnList);
	...
	)
{
	...
	if (WorkContext)
	{
		status = FEraseSecureDeleteFile(WorkContext->FileName);
	...
	)
{
	...
	status = FEraseContextInit(&uniFileName, &volume, &context);
	if (!NT_SUCCESS(status))
	{
		PT_DBG_FILE("Status 0x%X %s:%d", status, __FILE__, __LINE__);
		goto _error;
	...
	if(isAutoErase == 0)
	{
		...
		DgiSetFileAttribute(&uniFileName, FILE_ATTRIBUTE_NORMAL);
		...
		{
			status = FEraseEraseFileByBlocks(&context, count, mask);

			if(status == STATUS_END_OF_FILE)
			{				
				status = FEraseEraseFile(&context, count, mask);
			}
		}
	...
	{
		status = FEraseDeleteEmptyDir(&objAttr);
		if (!NT_SUCCESS(status))
		{
			PT_DBG_FILE("Status 0x%X %s:%d", status, __FILE__, __LINE__);
			PT_DBG_PRINT(PTDBG_TRACE_VALUE, ("FileErase!%s FEraseDeleteEmptyDir 0x%X\n", __FUNCTION__, status));
		}		
	}
	else
	{
	...
	)
{
	...
	while((status = ZwFsControlFile(
		...
		sizeof(STARTING_VCN_INPUT_BUFFER),
		pbub,
		(ULONG)sizeBuffer)) == STATUS_BUFFER_OVERFLOW || status == STATUS_PENDING)
	{
		if(status == STATUS_PENDING)
		{
	...
	)
{
	...
	//
	//// Пишем только один байт в конец
	//status = FEraseSecureOverwrite(fHandle, &position, &Mask, sizeof(UCHAR), Count);
	//if(!NT_SUCCESS(status)) {
	//	PT_DBG_FILE("Status 0x%X %s:%d", status, __FILE__, __LINE__);
	//	goto _error;
	//}
	...
	while(position.CurrentByteOffset.QuadPart < Context->NetInfoFile.AllocationSize.QuadPart)
	{
		...
		status = FEraseSecureOverwrite(fHandle, &position, buffer, bytesToWrite, Count);
		if(!NT_SUCCESS(status))
		{
			break;
		}
	...
	)
{
	...
	status = FEraseGetFileClusters(fHandle, &vcnList);
	...
	for(ULONG index = 0; index < vcnList->ExtentCount; ++index)
	{
		...
		if(lcn.QuadPart != LLINVALID)
		{
			...
			for(ULONG clusterCount = 0; clusterCount < (ULONG)(vcnList->Extents[index].NextVcn.QuadPart - startVcn.QuadPart); ++clusterCount)
			{
				...
				for(UINT32 passes = 1; passes <= Count; ++passes)
				{
					PIRP IrpFsd = IoBuildSynchronousFsdRequest(
						IRP_MJ_WRITE,
	...
	)
{
	...
	if(isErase)
	{
		...
		status = FltQueryInformationFile(
			Data->Iopb->TargetInstance,
			Data->Iopb->TargetFileObject,
			&contextErase.InfoFile,
			sizeof(contextErase.InfoFile),
		...
		{
			status = FEraseFltEraseFileByBlocks(&contextErase, count, mask);

			if(status == STATUS_END_OF_FILE)
			{
				status = FEraseFltEraseFile(&contextErase, count, mask);
			}
		}
		else
		{
