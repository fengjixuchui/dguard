...
NTSTATUS SziDosNameToNtNameVolume(
	...
	)
{
	...
	__try {
		...
		if (Size) {
			status = SziCreateUnicodeStringFromANSIString(OutString, buffer, Tag);
		}
	}__except(1)
	{
		status = STATUS_UNSUCCESSFUL;
	...
	)
{
	...
	if(NT_SUCCESS(status)) {
		...
		status = SziCreateUnicodeStringFromString(String, instanceId, Tag);
