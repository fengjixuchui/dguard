	...
	)
{
	...
	if (Sid != NULL) {
		status = SziConvertSidToUnicodeString(&newEvent->Sid, Sid, CF_EVENTSTRING_TAG);
		if (!NT_SUCCESS(status)) {
			DbgPrint("status 0x%X File %s:%d\n", status, __FILE__, __LINE__);
			goto _exit;
		}
	...
	)
{
	...
	for(;;)	{		
		...
		if(status == STATUS_TIMEOUT) {
			if(wskSocket != NULL) {
				SziSenderDisconnect(wskSocket);
				wskSocket = NULL;
			}
		}

		BOOLEAN isInit = SziSenderIsInit();
		if (isInit == FALSE) {
			status = SziSenderInit();
			isInit = NT_SUCCESS(status);
			DbgPrint("status 0x%X File %s:%d\n", status, __FILE__, __LINE__);
		}

		if(isInit && !SziEventIsEmpty()) {
			PEVENTSECURITY sEvent = NULL;

			if (wskSocket == NULL) {
				status = SziSenderConnect(gEventContext.SocketPort, &wskSocket);				
			} else {
				status = STATUS_SUCCESS;
			}			
	...
	)
{
	...
	status = SziBase64Encode((UCHAR*)bufferEvent, sizeEvent, bufferBase64, sizeBase64Max, &sizeBase64);
	if(!NT_SUCCESS(status)) {
		DbgPrint("status 0x%X File %s:%d\n", status, __FILE__, __LINE__);
		goto _error;
	}
	...
	status = SziSenderWrite(Socket, (PUCHAR)headEvent, sizeof(HEAD_EVENT));
	if(!NT_SUCCESS(status)) {
		DbgPrint("status 0x%X File %s:%d\n", status, __FILE__, __LINE__);
		goto _error;
	}

	status = SziSenderWrite(Socket, bufferBase64, sizeBase64);
	if(!NT_SUCCESS(status)) {
		DbgPrint("status 0x%X File %s:%d\n", status, __FILE__, __LINE__);
		goto _error;
	}
