...
#define	REG_PATH_MENU		L"%s\\shellex\\ContextMenuHandlers\\%s"
...
{
	HRESULT hr;
	wchar_t szCLSID[MAX_PATH] = L"\0";
	wchar_t szSubkey[MAX_PATH] = L"\0";
...
HRESULT RegisterShellExtContextMenuHandler(PCWSTR pszFileType, const CLSID& clsid, PCWSTR pszFriendlyName) 
{
	...
	hr = StringCchPrintf(szSubkey, ARRAYSIZE(szSubkey),	REG_PATH_MENU, pszFileType, szCLSID);
	if(SUCCEEDED(hr)) 
	{
